...
#hcp_rsfc_data <- read.csv("D:/OneDrive/GraduateStudent_Phd1/4_Population_differences_BrainParcellation/Population_differences_BrainParcellation/8_MSN_GLM_2groups_2nd/SEM/majorRe/pc1_signed/HCPYA_Edu_z_top100_v1.csv")  #OK
hcp_rsfc_data <- read.csv("D:/OneDrive/GraduateStudent_Phd1/4_Population_differences_BrainParcellation/Population_differences_BrainParcellation/8_MSN_GLM_2groups_2nd/SEM/majorRe/pca1_sp10_signed/HCPYA_Edu_z_top100_v1.csv") #OK
...
sem_model <- '
  ...
  # regressions
    RSFC_mean ~   age + sex+ c*group01 + b1*education + RMS + ICV
    education ~ a1*group01+ age +sex + RMS + ICV

    
    ab := (a1*b1)               # mediation effect (a*b path)

  
'
#fit <- sem(sem_model, data = data_combined, std.lv = TRUE)
